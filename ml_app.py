...
def load_kaggle_dataset(dataset_name, max_rows=5000):
    """Load dataset from Kaggle with error handling and cleanup"""
    try:
        # Import kaggle only when needed
        import kaggle
        
        # Set up Kaggle API configuration - check local .kaggle folder first
        local_kaggle_dir = ".kaggle"
        home_kaggle_dir = os.path.expanduser("~/.kaggle")
        
        # Check if kaggle.json exists in local .kaggle folder
        local_kaggle_file = os.path.join(local_kaggle_dir, "kaggle.json")
        home_kaggle_file = os.path.join(home_kaggle_dir, "kaggle.json")
        
        if os.path.exists(local_kaggle_file):
            # Use local kaggle.json file
            kaggle_file = local_kaggle_file
            st.info("✅ Using local kaggle.json file")
        elif os.path.exists(home_kaggle_file):
            # Use home directory kaggle.json file
            kaggle_file = home_kaggle_file
            st.info("✅ Using kaggle.json from home directory")
        else:
            # Try to copy from Downloads
            downloads_path = os.path.expanduser("~/Downloads")
            kaggle_files = [f for f in os.listdir(downloads_path) if f.startswith("kaggle") and f.endswith(".json")]
            
            if kaggle_files:
                # Create local .kaggle directory if it doesn't exist
                if not os.path.exists(local_kaggle_dir):
                    os.makedirs(local_kaggle_dir)
                
                # Copy kaggle.json from Downloads to local .kaggle directory
                source_file = os.path.join(downloads_path, kaggle_files[0])
                shutil.copy2(source_file, local_kaggle_file)
                kaggle_file = local_kaggle_file
                st.info("✅ Copied kaggle.json from Downloads to local .kaggle folder")
            else:
                raise Exception("No kaggle.json file found in Downloads folder or .kaggle directory")
        
        # Set KAGGLE_CONFIG_DIR to point to our local .kaggle folder
        ...
        # Load the first CSV file (or the largest one)
        if len(csv_files) == 1:
        ...
        # Limit rows if dataset is too large
        if len(df) > max_rows:
            df = df.head(max_rows)
            st.info(f"Dataset was large ({len(df)} rows), showing first {max_rows} rows for performance")
        ...
        # Clean up on error
        if os.path.exists("temp_kaggle_data"):
            shutil.rmtree("temp_kaggle_data")
        st.error(f"Failed to load Kaggle dataset {dataset_name}: {str(e)}")
        st.info("💡 Make sure you have a valid kaggle.json file in your Downloads folder")
        st.info("💡 Alternative: You can manually set Kaggle credentials using environment variables")
        return None
...
dataset_source = st.sidebar.radio("Choose dataset source:", ["Upload CSV", "OpenML", "Hugging Face", "Kaggle"])
...
with st.sidebar.expander("ℹ️ Dataset Info"):
    ...
    **🏆 Kaggle**: Real-world datasets from competitions
    - House prices, Titanic, Customer segmentation
    - Time series, Computer vision, NLP datasets
...
if dataset_source == "Upload CSV":
    ...
    # Enhanced popular Kaggle datasets with detailed information
    ...
    # Dataset selection method
    kaggle_selection_method = st.sidebar.radio("Choose selection method:", ["Popular Datasets", "Search Datasets", "Custom Dataset", "Download & Upload"])
    ...
    if kaggle_selection_method == "Popular Datasets":
        ...
        dataset_info = popular_kaggle_datasets[selected_kaggle_dataset]
        kaggle_name = dataset_info["ref"]
        ...
        with st.sidebar.expander("📊 Dataset Info"):
            ...
            st.write(f"**Downloads:** {dataset_info['downloads']}")
            st.write(f"**Votes:** {dataset_info['votes']}")
            st.write(f"**Tags:** {dataset_info['tags']}")
        
        st.sidebar.caption(f"Dataset: {kaggle_name}")
        
    elif kaggle_selection_method == "Search Datasets":
        search_query = st.sidebar.text_input("Enter search terms:", placeholder="e.g., 'machine learning', 'stock prices'")
        ...
        if st.sidebar.button("🔍 Search"):
            ...
            if search_results and len(search_results) > 0:
                st.sidebar.success(f"Found {len(search_results)} datasets!")
        ...
        if 'kaggle_search_results' in st.session_state:
            ...
            for i, dataset in enumerate(st.session_state['kaggle_search_results'][:5]):  # Show top 5
                with st.sidebar.expander(f"📊 {dataset.get('title', 'Untitled')[:30]}..."):
                    st.write(f"**Description:** {dataset.get('subtitle', 'No description')[:100]}...")
                    ...
                    if dataset.get('tags'):
                        tags = [tag['name'] for tag in dataset['tags'][:3]]
                        st.write(f"**Tags:** {', '.join(tags)}")
                    
                    dataset_ref = f"{dataset.get('owner')}/{dataset.get('name')}"
                    
                    if st.button(f"🚀 Load & Run ML", key=f"search_load_{i}"):
    ...
    elif kaggle_selection_method == "Download & Upload":
        st.sidebar.subheader("📥 Download & Upload Method")
        st.sidebar.write("**Step 1:** Get download links for datasets")
        st.sidebar.write("**Step 2:** Download manually from Kaggle")
        st.sidebar.write("**Step 3:** Upload the CSV file here")
        
        # Show popular datasets with download links
        st.sidebar.write("**Popular Datasets with Download Links:**")
        
        for name, info in popular_kaggle_datasets.items():
            with st.sidebar.expander(f"📊 {name}"):
                ...
                st.write(f"**Dataset Page:** [View on Kaggle]({kaggle_url})")
                
                # Create a download button that opens in new tab
                download_html = f"""
                <a href="{kaggle_url}" target="_blank" 
                   style="background-color: #20beff; color: white; padding: 8px 16px; 
                          text-decoration: none; border-radius: 4px; display: inline-block; 
                          margin: 5px 0; font-weight: bold; font-size: 14px;"
                   onclick="window.open('{kaggle_url}', '_blank'); return false;">
                    ⬇️ Download Dataset
                </a>
                """
                st.markdown(download_html, unsafe_allow_html=True)
                st.write("**Instructions:**")
                st.write("1. Click 'Download Dataset' button above")
                st.write("2. On Kaggle page, click the 'Download' button")
                st.write("3. Save the zip file to your computer")
                st.write("4. Extract the CSV file from the zip")
                st.write("5. Upload the CSV file below")
        
        # File uploader for downloaded CSV
        st.sidebar.subheader("📤 Upload Downloaded CSV")
        uploaded_file = st.sidebar.file_uploader("Upload your downloaded CSV file", type=["csv"], key="kaggle_upload")
        ...
        kaggle_name = st.sidebar.text_input("Enter Kaggle dataset name", "c/house-prices-advanced-regression-techniques")
        st.sidebar.caption("👉 Format: username/dataset-name or competition/dataset-name")
    
    # Max rows setting
    max_rows = st.sidebar.slider("Max rows to load", 1000, 10000, 5000, 1000)
    st.sidebar.caption("💡 Larger datasets will be truncated for performance")
    
    if kaggle_selection_method not in ["Search Datasets", "Download & Upload"] and st.sidebar.button("Load from Kaggle"):
        with st.spinner(f"🔄 Loading dataset {kaggle_name}..."):
            # Try direct download first, fallback to API
            try:
                zip_content = download_dataset_direct(kaggle_name)
                if zip_content:
                    csv_content, filename = extract_csv_from_zip(zip_content)
                    if csv_content:
                        df = pd.read_csv(io.StringIO(csv_content.decode('utf-8')))
                        if len(df) > max_rows:
                            df = df.head(max_rows)
                        st.success(f"✅ Successfully loaded dataset {kaggle_name} (Direct Download)")
                    else:
                        raise Exception("No CSV files found")
                else:
                    raise Exception("Direct download failed")
            except Exception as e:
                st.warning(f"⚠️ Direct download failed: {str(e)}")
                st.info("🔄 Trying API method as fallback...")
                
                # Fallback to API method
                try:
                    df = load_kaggle_dataset(kaggle_name, max_rows)
                    if df is None:
                        st.error("❌ Both direct download and API methods failed")
                        st.info("💡 Try the 'Download & Upload' method instead:")
                        st.info("1. Go to the dataset on Kaggle")
                        st.info("2. Download manually")
                        st.info("3. Upload the CSV file using 'Download & Upload' option")
                    else:
                        st.success(f"✅ Successfully loaded dataset {kaggle_name} (API Fallback)")
                except Exception as api_error:
                    st.error(f"❌ API method also failed: {str(api_error)}")
                    st.info("💡 Please use the 'Download & Upload' method for reliable access to Kaggle datasets")

# Enhanced Kaggle Section in Main Area
if dataset_source == "Kaggle":
    st.header("🏆 Enhanced Kaggle Dataset Explorer")
    
    # Show current selection method
    if kaggle_selection_method == "Download & Upload":
        st.subheader("📥 Download & Upload Method")
        st.info("""
        **This method provides a reliable way to use Kaggle datasets:**
        
        1. **Browse Popular Datasets** - See detailed information about each dataset
        2. **Get Direct Links** - Click the Kaggle links to open datasets in your browser
        3. **Download Manually** - Download the dataset from Kaggle's website
        4. **Upload to App** - Upload the CSV file using the sidebar uploader
        5. **Run ML Pipeline** - Use your complete AutoML pipeline on the data
        
        **Benefits:**
        - ✅ **100% Reliable** - No API issues or download failures
        - ✅ **Full Control** - You choose which files to use from the dataset
        - ✅ **No API Keys** - Works without Kaggle authentication
        - ✅ **Complete Pipeline** - Full AutoML functionality
        """)
        
        # Show popular datasets in main area with download links
        st.subheader("📊 Popular Datasets with Download Links")
        ...
        for name, info in popular_kaggle_datasets.items():
            with st.expander(f"📊 {name}"):
                ...
                with col1:
                    ...
                    download_html = f"""
                    <a href="{kaggle_url}" target="_blank" 
                       style="background-color: #20beff; color: white; padding: 10px 20px; 
                    ...
                    st.write("3. Save the zip file to your computer")
                    st.write("4. Extract the CSV file from the zip")
                    st.write("5. Upload the CSV file in the sidebar")
                    st.write("6. Run your ML pipeline!")
                
                with col2:
                    st.markdown(f"[![Kaggle](https://img.shields.io/badge/Kaggle-Open%20Dataset-blue)]({kaggle_url})")
                    st.write("**Quick Stats:**")
                    st.metric("Downloads", info['downloads'])
                    st.metric("Votes", info['votes'])
        
        st.subheader("💡 Pro Tips")
        st.success("""
        **🎯 Best Practices:**
        - **Start with small datasets** like Titanic or Wine Quality for testing
        - **Check dataset size** before downloading (some can be very large)
        - **Look at the dataset description** to understand what you're working with
        - **Use the tags** to find datasets that match your interests
        - **Check download counts** to find popular, well-maintained datasets
        """)
        
        # Skip the rest of the Kaggle section for Download & Upload method
        st.stop()
    
    # Show popular datasets in main area
    st.subheader("📊 Popular Datasets with Detailed Information")
    
    # Display popular datasets in main area
    for name, info in popular_kaggle_datasets.items():
        with st.expander(f"📊 {name}"):
            col1, col2 = st.columns([3, 1])
            ...
            with col2:
                if st.button(f"🚀 Load & Run ML", key=f"main_{info['ref']}"):
                    with st.spinner(f"Downloading {name}..."):
                        zip_content = download_dataset_direct(info['ref'])
    ...
    # Search functionality in main area
    st.subheader("🔍 Search Custom Datasets")
    
    search_query_main = st.text_input("Enter search terms (e.g., 'machine learning', 'stock prices', 'customer data'):", 
                            placeholder="Type your search here...", key="main_search")
    
    if st.button("🔍 Search Datasets", type="primary", key="main_search_btn") and search_query_main:
        with st.spinner("Searching Kaggle datasets..."):
            results = search_kaggle_datasets(search_query_main)
            ...
            for i, dataset in enumerate(results):
                with st.expander(f"📊 {dataset.get('title', 'Untitled')} by {dataset.get('owner', 'Unknown')}"):
                    ...
                    with col1:
                        ...
                        description = dataset.get('subtitle', 'No description available')
                        if not description or description == 'No description available':
                            description = f"Dataset by {dataset.get('owner', 'Unknown')} - {dataset.get('title', 'Untitled')}"
                        ...
                        if dataset.get('tags'):
                            tags = [tag['name'] for tag in dataset['tags'][:8]]  # Show more tags
                            st.write(f"**Tags:** {', '.join(tags)}")
                        else:
                            st.write("**Tags:** No tags available")
                        
                        # Show additional info if available
                        if dataset.get('usabilityRating'):
                            st.write(f"**Usability Rating:** {dataset.get('usabilityRating')}/5")
                        
                        if dataset.get('lastUpdated'):
                            st.write(f"**Last Updated:** {dataset.get('lastUpdated')}")
                    
                    with col2:
                        dataset_ref = f"{dataset.get('owner')}/{dataset.get('name')}"
                        
                        if st.button(f"🚀 Download & Run ML", key=f"main_search_{i}"):
                            with st.spinner(f"Downloading {dataset.get('title', 'dataset')}..."):
                                zip_content = download_dataset_direct(dataset_ref)
